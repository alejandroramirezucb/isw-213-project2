...
export class ControladorRegistro extends ControladorBase {
  async manejar(req, res) {
    ...
    if (!this.validarCampos(datos, requeridos)) {
      return this.responder(res, CodigosHttp.PETICION_INVALIDA, { error: 'Faltan campos requeridos' });
    }
    ...
    try {
      ...
      if (authResult.status >= CodigosHttp.PETICION_INVALIDA) {
      ...
      this.responder(res, CodigosHttp.ERROR_SERVIDOR, { error: `Error de conexión con Supabase: ${error.message}` });
    }
  }
  ...
  async _crearPerfil(res, datos, usuarioId) {
    ...
    if (perfilResult.status >= CodigosHttp.MINIMO_REDIRECCION) {
      await this.cliente.delete(`/auth/v1/admin/users/${usuarioId}`);
      return this.responder(res, CodigosHttp.ERROR_SERVIDOR, { error: this.extraerMensajeError(perfilResult.data) });
    }
    ...
    if (authRow.status >= CodigosHttp.MINIMO_REDIRECCION) {
      await this.cliente.delete(`/auth/v1/admin/users/${usuarioId}`);
      return this.responder(res, CodigosHttp.ERROR_SERVIDOR, { error: this.extraerMensajeError(authRow.data) });
    }
