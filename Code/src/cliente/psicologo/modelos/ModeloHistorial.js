...
export class ModeloHistorial {
  ...
  async cargar(busqueda = '') {
    const historial = await this._repositorioCitas.obtenerHistorialPacientes(this._psicologoId);
    const filtrado = busqueda
      ? historial.filter((item) =>
          `${item.paciente.nombre} ${item.paciente.apellido}`.toLowerCase().includes(busqueda.toLowerCase()),
        )
      : historial;
    document.dispatchEvent(new CustomEvent('psicologo:historialCargado', { detail: { historial: filtrado } }));
  }

  async toggleBloqueo(pacienteId, bloqueadoActual) {
    const exito = await this._repositorioPacientes.actualizarBloqueo(pacienteId, !bloqueadoActual);
    if (exito) {
      this._repositorioCitas.invalidarCacheHistorial(this._psicologoId);
      document.dispatchEvent(new CustomEvent('psicologo:mensaje', {
        detail: { texto: `Paciente ${!bloqueadoActual ? 'bloqueado' : 'desbloqueado'} exitosamente`, tipo: 'exito' },
      }));
      document.dispatchEvent(new CustomEvent('psicologo:bloqueoActualizado'));
    } else {
      document.dispatchEvent(new CustomEvent('psicologo:mensaje', {
        detail: { texto: 'Error al actualizar estado del paciente', tipo: 'error' },
      }));
    }
  }
  ...
  exportarPdf(item) {
    ...
    if (!pdfLib?.jsPDF) {
      return document.dispatchEvent(new CustomEvent('psicologo:mensaje', { detail: { texto: 'No se pudo generar el PDF', tipo: 'error' } }));
    }
    const { jsPDF } = pdfLib;
    ...
    if (doc.autoTable) {
      doc.autoTable({ startY: 55, head: [['Fecha', 'Hora', 'Estado']], body: rows, margin: { left: m, right: m }, headStyles: { fillColor: [66, 133, 244], textColor: 255, halign: 'center' }, bodyStyles: { halign: 'center' } });
    }
