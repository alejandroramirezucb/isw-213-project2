class GestorHistorial {
  ...
  static #mostrarDetalle(item) {
    Fachada.abrirModal('modal-historial-paciente');
    ...
    item.citas.forEach((cita) => {
      ...
      html += `<div class="cita-item">
        <div class="cita-item__info">
          <span class="cita-item__fecha">${Fachada.formatearFechaCorta(new Date(cita.fecha + 'T00:00:00'))}</span>
          <span class="cita-item__hora">${Fachada.formatearHora(cita.hora)}</span>
        </div>
  ...
  static #descargarHistorial(item) {
    ...
    item.citas.forEach((cita) => {
      lineas.push(
        `${Fachada.formatearFecha(new Date(cita.fecha + 'T00:00:00'))}\t${Fachada.formatearHora(cita.hora)}\t${cita.estado}`,
      );
    });
